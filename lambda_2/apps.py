...
def lambda_handler(event,context):
    ...
    for house in results['hits']['hits']:
        ...
        barrio = house['_source']['listing']['locations']['neighbourhoods'][0]['name']
        nhabs = house['_source']['listing']['rooms']['id']
        nbath = house['_source']['listing']['baths']['id']
        
        csv += str(dt.date())+","+str(barrio)+","+str(price)+","+str(nhabs)+","+str(nbath)+","+str(mt2)+"\n"

    s3 = boto3.resource('s3')
    s3.Object('zappa-tmtvhlyzs', "casas-final-"+str(dt.date())+'.csv').put(Body=csv)
